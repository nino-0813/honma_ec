          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {cartItems.map((item) => (
              <div key={`${item.product.id}-${item.variant || 'default'}`} className="flex gap-4 pb-4 border-b border-gray-100 last:border-b-0">
                <Link to={`/products/${item.product.handle}`} onClick={onClose} className="flex-shrink-0 aspect-square w-20 bg-gray-100 rounded overflow-hidden block">
                  <FadeInImage 
                ...
                <div className="flex-1 min-w-0">
                  <Link to={`/products/${item.product.handle}`} onClick={onClose}>

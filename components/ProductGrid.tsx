    ...
    <section className="py-8 md:py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {!loading && !error && displayProducts.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-10 md:gap-x-6 md:gap-y-12">
          {displayProducts.map((product, index) => {
            ...
            return (
          <Link key={product.id} to={productPath} className="group flex flex-col opacity-0 animate-fade-in-up" style={{ animationDelay: `${index * 100}ms` }}>
      ...
      {!loading && !error && displayProducts.length > 0 && (
        <div className="text-center mt-12">
           <Link to="/collections" className="inline-block bg-black text-white px-10 py-3 text-xs tracking-[0.2em] hover:opacity-80 transition-opacity uppercase">
             View all
           </Link>
        </div>
      )}

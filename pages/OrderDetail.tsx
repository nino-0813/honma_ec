...
const OrderDetail = () => {
  ...
  const getOrderStatusText = (status: string) => {
    ...
    const statusMap: { [key: string]: string } = {
      ...
      processing: '発送前',
      shipped: '発送済み',
      delivered: '発送済み', // delivered も「発送済み」に統合
      cancelled: 'キャンセル',
    };
        ...
        <main className="min-h-screen bg-white pt-24 pb-16">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center py-16">
              ...
              <Link to="/mypage?tab=orders">
                <a className="text-sm text-primary hover:underline">購入履歴に戻る</a>
              </Link>
            </div>
          </div>
        </main>
    ...
    <>
      ...
      <main className="min-h-screen bg-white pt-24 pb-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-6">
            <Link to="/mypage?tab=orders">
              <a className="text-sm text-gray-600 hover:text-gray-900 flex items-center gap-2">
          ...
          {/* 注文番号とステータス */}
          <div className="mb-8">
            <h1 className="text-2xl md:text-3xl font-serif font-medium text-gray-900 mb-2">
              注文詳細
            </h1>
                  ...
                  <div key={item.id} className="flex gap-4 pb-4 border-b border-gray-200 last:border-0 last:pb-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-medium text-gray-900">
                        ¥{item.product_price.toLocaleString()} × {item.quantity} = ¥{item.line_total.toLocaleString()}
                      </p>
                    </div>
                  </div>
                ))}
                  ...
                  {order.order_items.map((item, index) => (
                    <div key={item.id} className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        {/* 発送済みのときだけ配送会社/発送番号を表示 */}
                        {(order.order_status === 'shipped' || order.order_status === 'delivered') && (order.shipping_carrier || order.tracking_number) && (
                          <div className="mt-2 text-xs text-gray-700 space-y-1">
                            {order.shipping_carrier && (
                              <div>配送会社: <span className="font-medium text-gray-900">{order.shipping_carrier}</span></div>
                            )}
                            {order.tracking_number && (
                              <div>発送番号: <span className="font-medium text-gray-900">{order.tracking_number}</span></div>
                            )}
                          </div>
                        )}
                      </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-1 text-sm text-gray-600">
                    ...
                    {/* shipping_address は既に「都道府県+市区町村+町名・番地(+建物)」の形で保存されているため、二重表示を避ける */}
                ...
                <div>
                  ...
                  <div className="space-y-1 text-sm text-gray-600">
                    ...
                    {/* 購入者情報の住所が保存されていない場合は配送先住所を表示（既存データの互換性のため） */}
                    {!order.billing_postal_code && !order.billing_city && !order.billing_address && (
                      <>
                        {order.shipping_address && <p>{order.shipping_address}</p>}
                      </>
                    )}
                    <p>{order.last_name} {order.first_name}様</p>
                    {order.phone && <p>{order.phone}</p>}
                  </div>
                </div>
              </div>

...
const ProductDetail = () => {
  ...
  // NOTE: 画像表示は FadeInImage に任せる（白抜け防止 & 最適化）
  ...
  if (!loading && error) return <div className="h-screen flex items-center justify-center text-center"><p className="text-red-500 mb-2">エラー</p><p className="text-sm text-gray-500 mb-4">{error}</p><Link to="/collections" className="text-primary hover:underline">一覧に戻る</Link></div>;
  if (!loading && !product) return <div className="h-screen flex items-center justify-center text-center"><p className="text-gray-500 mb-4">商品なし</p><Link to="/collections" className="text-primary hover:underline">一覧に戻る</Link></div>;
              ...
              <div className="border-t border-gray-200 mt-8">
                ...
                <div className="py-4 border-b border-gray-200">
                  ...
                  <div className={`overflow-hidden transition-all duration-500 ease-in-out ${activeAccordion === 'shipping' ? 'max-h-[200px] opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
                    <div className="pb-4"><Link to="/legal" className="text-sm text-gray-600 hover:text-black underline transition-colors">特定商取引法に基づく表記</Link></div>
                  </div>
                </div>
                <div className="py-4 border-b border-gray-200">

...
const ProductDetail = () => {
        ...
        if (fetchError && fetchError.code === 'PGRST116') {
          ...
          if (anyStatusError) {
            ...
            const extendedProduct = {
               ...dbProduct,
               variants_config: anyStatusData.variants_config
            } as Product;
          ...
          const extendedProduct = {
             ...dbProduct,
             variants_config: data.variants_config
          } as Product;
          setProduct(extendedProduct);
          setError(null);
        } else {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900 mx-auto mb-4"></div></div>;
  if (!loading && error) return <div className="h-screen flex items-center justify-center text-center"><p className="text-red-500 mb-2">エラー</p><p className="text-sm text-gray-500 mb-4">{error}</p><Link href="/collections"><a className="text-primary hover:underline">一覧に戻る</a></Link></div>;
              ...
              <div className="space-y-4 mb-12">
                 {!product.soldOut && (
                   <div className="flex items-center justify-between border border-gray-200 p-1 max-w-[140px] mb-6">
                      <button type="button" onClick={(e) => { e.preventDefault(); setQuantity(Math.max(1, quantity - 1)); }} className="w-10 h-10 flex items-center justify-center text-gray-500 hover:text-black transition-colors">−</button>
                      <span className="text-sm font-serif w-8 text-center">{quantity}</span>
                      <button type="button" onClick={(e) => { e.preventDefault(); setQuantity(quantity + 1); }} className="w-10 h-10 flex items-center justify-center text-gray-500 hover:text-black transition-colors">+</button>
                   </div>
                 )}
                 {product.soldOut ? (
                ...
                <div className="py-4 border-b border-gray-200">
                  ...
                  <div className={`overflow-hidden transition-all duration-500 ease-in-out ${activeAccordion === 'desc' ? 'max-h-[5000px] opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
                    <div className="text-sm leading-loose text-gray-600 font-light space-y-4 pb-4">
                      ...
                      <p className="text-xs text-gray-500 mt-4">※イケベジのパンフレットを1部同封しておりますが、プレゼント用等でパンフレットを複数ご希望の方は、「備考」欄に必要部数を記載いただければと思います。（最大5部）</p>
                    </div>
                  </div>
                </div>
                <div className="py-4 border-b border-gray-200">
                  ...
                  <div className={`overflow-hidden transition-all duration-500 ease-in-out ${activeAccordion === 'shipping' ? 'max-h-[200px] opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
                    <div className="pb-4"><Link href="/legal"><a className="text-sm text-gray-600 hover:text-black underline transition-colors">特定商取引法に基づく表記</a></Link></div>
                  </div>
                </div>
                <div className="py-4 border-b border-gray-200">
